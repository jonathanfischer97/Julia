...
function make_fitness_function_with_fixed_inputs(evalfunc::Function, prob::ODEProblem, fixed_input_triplet::Vector{Float64}, triplet_idxs::Tuple{Int, Int, Int})
    function fitness_function(input::Vector{Float64})
        new_input = [i for i in eachindex(input)]
...
function fixed_triplet_csv_maker(param1::String, param2::String, param3::String, constraints::ConstraintType, prob::ODEProblem)
    ...
    fixed_values1 = range(fixedtrip[1].min, stop = fixedtrip[1].max, length = 100)
    fixed_values2 = range(fixedtrip[2].min, stop = fixedtrip[2].max, length = 100)
    fixed_values3 = range(fixedtrip[3].min, stop = fixedtrip[3].max, length = 100)
    ...
    for val1 in fixed_values1
        for val2 in fixed_values2
            for val3 in fixed_values3
                ...
                oscillatory_points_df = run_GA(fixed_ga_problem, make_fitness_function_closure; population_size = 5000, iterations = 8) 

                average_period = mean(oscillatory_points_df.per)
                average_amplitude = mean(oscillatory_points_df.amp)
...
results_df = fixed_triplet_csv_maker("ka1", "ka2", "ka3", param_constraints, ogprob)