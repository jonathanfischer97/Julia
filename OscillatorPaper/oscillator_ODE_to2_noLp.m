...
% time=[0,100]
function[timePts, cs]=oscillator_ODE_to2_noLp(parmVec, ICvec, time)
...
opt=odeset('RelTol',1E-7,'AbsTol',1E-9);
 [timePts,conc] = ode15s(@(t,y) odes_2(t,y,parmVec, mass),time,y0,opt);
...
cs.Lp=zeros(N,1);
for i=1:1:N   
    [cs.LpAK(i), cs.LK(i), cs.P(i), cs.LpP(i), cs.LpAP(i),cs.Lp(i)]=calc_other_vars(conc(i,:),parmVec, mass);
    varVec.L=cs.L(i);
    varVec.LK=cs.LK(i);
    ...
    varVec.Lp=cs.Lp(i);
    varVec.LpA=cs.LpA(i);
    [cs.A(i), cs.K(i), cs.LpAKL(i), cs.LpAPLp(i)]=calculate_mass_vars(mass, varVec);%this is unaffected by fixed vs unknown vars once varVec is correct
end
...
cs.A(1)
 ...
 plot(timePts, cs.LpAP,'m--','LineWidth',2)%LpA
  plot(timePts, cs.LpAKL,'g--','LineWidth',2)%LpAKL
   ...
   plot(timePts, cs.LpA,'cp','LineWidth',2)%LpAPLp
 
 legend('LpAK','LpP','LK','LpAP','LpAKL','LpAPLp','K','P','LpA')
...
%ODE DEFINITIONS
function dy = odes_2(t,y,parmVec, mass)
...
[LpAK, LK, P, LpP, LpAP,Lp]=calc_other_vars(y, parmVec,mass);
...
dy(2) = kb3*LpAK + kb4*LpAP + ka2*A*Lp - kb2*y(2) - ka3*K*y(2) - ka4*y(2)*P;%LpA
...
function[LpAK, LK, P, LpP, LpAP,Lp]=calc_other_vars(y, parmVec,mass)
    ...
    %set dL=0, solve for L
    Lp=eval_Lproot1(y, parmVec, mass);
