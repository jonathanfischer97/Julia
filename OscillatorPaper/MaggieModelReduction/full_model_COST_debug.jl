...
function oscillatory_strength(sol::ODESolution; min_freq=0.02, max_freq=0.5)
...
fitness_function = make_fitness_function(eval_fitness, prob) # Create a fitness function that includes your ODE problem as a constant
...
"""Test cost function"""
function oscillatory_strength(t::StepRangeLen, sol::Vector; min_freq=0.02, max_freq=0.5)
    # Extract the time points and solution values
    time_points = collect(t)
    solution = sol
...
function oscillatory_strength_biased(t, sol::Vector, target_freq::Float64, bias_sigma::Float64=0.1)
    solution = sol
    time_points = collect(t)
...
t = range(0, stop=10π, length=1000)
sine_wave = sin.(2π * 0.01 * t)


strength_sine_wave = oscillatory_strength(t, sine_wave, min_freq = 0.05, max_freq = 0.1)
biased_strength_sine_wave = oscillatory_strength_biased(t, sine_wave, 0.07, 0.9)
println("Oscillatory strength of sine wave: ", strength_sine_wave)
println("Biased oscillatory strength of sine wave: ", biased_strength_sine_wave)

# Test with a random signal (non-oscillatory signal)
random_signal = rand(1000)

strength_random_signal = oscillatory_strength(t, random_signal, min_freq= 0.05, max_freq = 0.1)
println("Oscillatory strength of random signal: ", strength_random_signal)

# Plot the test signals
plot(t, sine_wave, label="Sine wave", xlabel="Time", ylabel="Amplitude")
plot!(t, random_signal, label="Random signal")